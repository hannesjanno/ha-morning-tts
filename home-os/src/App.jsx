...
const floors = {
  1: {
    ...
    openPassages: [
      ...
      { x1:290.25, y1:188, x2:290.25, y2:269.1 },
    ],
...
function FloorPlan({floor}){const[selected,setSelected]=useState(null),room=floors[floor].rooms.find(it=>it.id===selected);return <div className="floor-plan"><FloorSvg key={floor} floor={floor} selected={selected} onSelect={id=>setSelected(id===selected?null:id)}/><div className="room-detail">{room?<><span>VALITUD RUUM</span><strong>{room.name}</strong><small>{room.area} · siia lisame hiljem Home Assistanti olekud ja juhtimise</small></>:<><span>INTERAKTIIVNE PLAAN</span><strong>Puuduta ruumi</strong><small>2D plaan sisaldab nüüd ka parkla, väravad, Wallboxi, terrassi ja varjualust.</small></>}</div></div>}
